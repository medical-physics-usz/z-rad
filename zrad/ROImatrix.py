...
class Matrix(object):
    def __init__(self, imap, rs_type, structure, map_name, XcontourPoints, YcontourPoints, Xcontour_WPoints, Ycontour_WPoints, Xcontour_Rec, Ycontour_Rec, columns, rows, HUmin, HUmax, binSize, bits, outlier_correction, HUmask, cropStructure = {"crop": False}):
        ...
        try:
            ...
            if structure != 'none': #if the structure to be anylsed was defined
                ...
                for i in arange(0, len(imap)): #slices
                    for j in arange(ymin, ymax+1): #rows
                        for k in arange(xmin, xmax+1): #columns
                            try:  #each point
                ...
                if Xcontour_Rec != []:
                    for i in arange(0, len(Xcontour_Rec)): #slices
                        for j in arange(0, len(Xcontour_Rec[i])): #sub-structres in the slice
                            for k in arange(0, len(Xcontour_Rec[i][j])): #each point
                                try:
