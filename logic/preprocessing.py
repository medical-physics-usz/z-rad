...
class Preprocessing:
    ...
    def __init__(self, load_dir, folder_prefix, start_folder, stop_folder, list_of_patient_folders,input_data_type,
                 dicom_structures, nifti_image, nifti_structures, mask_interpolation_method,
        ...
        def list_folders_in_range(start_folder, stop_folder, directory_path):

            items = [item for item in sorted(os.listdir(directory_path)) if
                     os.path.isdir(os.path.join(directory_path, item))]

            try:
                start_index = items.index(start_folder)
                stop_index = items.index(stop_folder)
        ...
        self.list_of_patient_folders = [self.folder_prefix + str(patient) for patient in list_of_patient_folders] if list_of_patient_folders else \
            list_folders_in_range(self.folder_prefix + str(start_folder), self.folder_prefix + str(stop_folder), self.load_dir)
    ...
    def load_patient(self, patient_number):
        ...
        self.resampling()
        print('SAVE AS', self.output_data_type)
    ...
    def process_nifti_files(self):
        ...
        if self.nifti_structures != ['']:
            for nifti_file in self.nifti_structures:
                key = 'MASK_' + nifti_file
                self.pat_original_image_and_masks[key] = self.extract_nifti(key, nifti_file)

    def extract_nifti(self, key, mask_file=None):
            ...
            for dicom_file in os.listdir(self.patient_folder):
                ...
                if dcm_data.Modality == 'RTSTRUCT':
                    ...
                    for ROI in self.dicom_structures:
                        ...
                        self.pat_original_image_and_masks[key] = Image(array=mask_ROI.tobytes(),
                                                                                     origin=
                                                                                     self.pat_original_image_and_masks[
                                                                                         'IMAGE'].origin,
                                                                                     spacing=
    ...
    def resampling(self):
        ...
        def resampled_origin(input_shape, input_spacing, output_spacing, input_origin, axis=0):

            """https://arxiv.org/pdf/1612.07003.pdf QCY4"""

            n_a = float(input_shape[axis])
            s_a = input_spacing[axis]
            s_b = output_spacing[axis]
            n_b = np.ceil((n_a * s_a) / s_b)
            x_b = input_origin[axis] + (s_a * (n_a - 1) - s_b * (n_b - 1)) / 2
            return x_b
        ...
        output_origin = [resampled_origin(input_shape, input_spacing, output_spacing, input_origin, axis) for axis in
                         range(3)]

        output_shape = np.ceil((np.array(input_shape) * (input_spacing / output_spacing))).astype(int)

        for key in list(self.pat_original_image_and_masks.keys()):

            sitk_image = sitk.GetImageFromArray(np.frombuffer(self.pat_original_image_and_masks[key].array,
                                                              dtype = self.pat_original_image_and_masks[key].dtype).reshape(input_shape[::-1]))
            ...
            resample.SetInterpolator(self.get_interpolator(key))

            print('START RESAMPLING')
            resampled_sitk_image = resample.Execute(sitk_image)
            print('Image', key, 'was resized')
            ...
            self.pat_resampled_image_and_masks[key] = Image(array=resampled_array,
                                                            origin=output_origin,
                                                            spacing=output_spacing,
                                                            direction=input_direction,
                                                            shape=output_shape,
                                                            dtype=None)
            del self.pat_original_image_and_masks[key]
    def get_interpolator(self, key):
        interpolators = {
    ...
    def save_as_nifti(self):
        ...
        for key, Img in self.pat_resampled_image_and_masks.items():
            print(key)
            output_path = os.path.join(self.save_dir, self.patient_number, key + '.nii.gz')
    ...
    def save_as_dicom(self):
        ...
        def write_slices(series_tag_values, new_img, i, spacing_list):
        ...
        for key, Img in self.pat_resampled_image_and_masks.items():
            if key.startswith('MASK'):
                ...
                rtstruct_save.add_roi(mask=roi_mask, name=key[5:])
