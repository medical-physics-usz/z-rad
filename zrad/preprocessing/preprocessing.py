...
class Preprocessing:
    ...
    def __init__(
        ...
        if not (
            isinstance(resample_resolution, (float, int)) and resample_resolution > 0
        ):
            raise ValueError(
                f"Resample resolution {resample_resolution} must be a positive int or float."
            )
        if resample_dimension not in ["3D", "2D"]:
            raise ValueError(
                f"Resample dimension '{resample_dimension}' is not '2D' or '3D'."
            )
    ...
    def get_interpolator(interpolation_method):
        ...
        if interpolation_method not in interpolator_mapping:
            raise ValueError(
                f"Interpolation method '{interpolation_method}' is not supported."
            )
        return interpolator_mapping[interpolation_method]

    @staticmethod
    def calculate_resampled_origin(
        initial_shape, initial_spacing, resulted_spacing, initial_origin, axis=0
    ):
    ...
    def resample(self, image, image_type):
        ...
        if self.resample_dimension == "3D":
            ...
            raise ValueError(
                f"Resample dimension '{self.resample_dimension}' is not supported."
            )

        # Calculate output origin
        output_origin = [
            self.calculate_resampled_origin(
                image.shape, image.spacing, output_spacing, image.origin, axis
            )
            for axis in range(3)
        ]
        ...
        output_shape = np.ceil(
            (
                ...
                * (np.array(image.spacing) / np.array(output_spacing))
            )
        ).astype(int)
        ...
        resample_filter.SetInterpolator(
            self.get_interpolator(self.interpolation_method)
        )
    ...
    def process_resampled_mask(self, resampled_sitk_image):
        ...
        if self.interpolation_threshold is None:
            raise ValueError(
                "interpolation_threshold must be set when resampling a mask"
            )

        mask_array = sitk.GetArrayFromImage(resampled_sitk_image)
        mask_array = np.where(mask_array >= self.interpolation_threshold, 1, 0).astype(
            np.int16
        )
