...
class Export(object):
    def Preset(self, exportList, wv,local, path_save, save_as, perf_names, path_image):
        ...
        par_names = ['Mean', 'SD', 'COV', 'skewness', 'kurtosis', 'var', 'median', 'percentile10', 'percentile90', 'iqr', 'Hrange', 'mad', 'rmad', 'H_energy', 'H_entropy', 'rms', 'H_uniformity',
             ...
             'M_energy', 'M_entropy', 'M_contrast', 'M_correlation', 'M_homogenity', 'M_homogenity_n', 'M_idiff', 'M_idiff_n', 'M_variance', 'M_sum_average', 'M_sum_entropy', 'M_sum_variance', 'M_diff_entropy', 'M_diff_variance', 'M_IMC1', 'M_IMC2', 'M_MCC', 'M_joint_max',  'M_joint_average', 'M_diff_average', 'M_dissimilarity', 'M_inverse_variance', 'M_autocorrelation', 'M_clust_tendency', 'M_clust_shade', 'M_clust_prominence',
             'coarseness', 'neighContrast', 'busyness', 'complexity', 'strength',
             'len_intensityVar','len_intensityVar_n', 'len_sizeVar',  'len_sizeVar_n', 'len_sse' , 'len_lse', 'len_lgse', 'len_hgse', 'len_sslge', 'len_sshge', 'len_lslge', 'len_lshge', 'len_rpc', 'len_grey_lev_var',  'len_zone_size_var', 'len_size_entropy',
             'M_len_intensityVar','M_len_intensityVar_n', 'M_len_sizeVar',  'M_len_sizeVar_n', 'M_len_sse' , 'M_len_lse', 'M_len_lgse', 'M_len_hgse', 'M_len_sslge', 'M_len_sshge', 'M_len_lslge', 'M_len_lshge', 'M_len_rpc', 'M_len_grey_lev_var',  'M_len_zone_size_var', 'M_len_size_entropy',
             'intensityVar','intensityVar_n', 'sizeVar', 'sizeVar_n', 'sse' , 'lse', 'lgse', 'hgse', 'sslge', 'sshge', 'lslge', 'lshge', 'rpc', 'grey_lev_var', 'zone_size_var', 'size_entropy']
        if calcGLDZM:
            app = ['GLDZM_intensityVar','GLDZM_intensityVar_n', 'GLDZM_sizeVar', 'GLDZM_sizeVar_n', 'GLDZM_sse' , 'GLDZM_lse', 'GLDZM_lgse', 'GLDZM_hgse', 'GLDZM_sslge', 'GLDZM_sshge', 'GLDZM_lslge', 'GLDZM_lshge', 'GLDZM_rpc', 'GLDZM_grey_lev_var', 'GLDZM_zone_size_var', 'GLDZM_size_entropy']
            par_names = par_names + app
        if calcNGLDM:
            app = ['NGLDM_intensityVar','NGLDM_intensityVar_n', 'NGLDM_sizeVar', 'NGLDM_sizeVar_n', 'NGLDM_sse' , 'NGLDM_lse', 'NGLDM_lgse', 'NGLDM_hgse', 'NGLDM_sslge', 'NGLDM_sshge', 'NGLDM_lslge', 'NGLDM_lshge', 'NGLDM_grey_lev_var', 'NGLDM_zone_size_var', 'NGLDM_size_entropy','NGLDM_energy']
            par_names = par_names + app
    ...
    def ExportResults(self, final, final_file, par_names, perf_names, wave_names, wv, local):
        ...
        calcNGLDM = True  
            ...
            # texture paramaters are save in groups regarding perfusion maps and then the with the order from par_names list
        #rearange the final list so wavelets have the order 'HHH', 'HHL', 'HLH', 'HLL', 'LHH', 'LHL', 'LLH', 'LLL'
