...
class MaskResampler:
    ...
    def __init__(self, resolution, method='nearest_neighbor', partial_volume_threshold=0.5):
        self.resolution = tuple(_normalize_resolution(resolution))
        self.method = method
        self.partial_volume_threshold = partial_volume_threshold
...
def _resample_image(image, output_spacing, interpolation_method):
    ...
    output_origin = [
        Resampler._calculate_resampled_origin(image.shape, image.spacing, output_spacing, image.origin, axis)
        for axis in range(3)
    ]
    ...
    resample_filter.SetSize(output_shape.tolist())
    resample_filter.SetOutputPixelType(sitk.sitkFloat64)
    resample_filter.SetInterpolator(Resampler._get_interpolator(interpolation_method))
