...
class ExportExcel(object):
    ...
    def __init__(self, ifshape, path_save, name_shape_pt, start, stop, save_as, dict_parameters):
        if ifshape:
            ...
            shape = ''
        texture = pd.read_csv(path_save + os.sep + save_as + '.txt', sep="\t", header=0, index_col=False)
    ...
    def cleanup(self, df):
        def replaceList(df, featName):
            df.loc[:, featName] = df[featName].apply(
                lambda x: float(x[1:len(x)].split(",")[0]) if type(x) == str else np.nan).copy()
        ...
        for iMCC in MCC_features:
            df[iMCC] = df.loc[:, iMCC].apply(lambda x: x.real)
