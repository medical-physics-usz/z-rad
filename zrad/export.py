...
def preset(wv, path_save, save_as, perf_names):
    try:
        makedirs(path_save)
    except OSError:
...
def export_excel(if_shape, path_save, save_as, dict_parameters):
    ...
    # these cols have entries as lists
    pd.options.mode.chained_assignment = None  # default='warn'
    for featName in ['voxels', 'vmin', 'vmax']:
        texture[featName] = texture[featName].apply(
            lambda x: eval(x)[0] if isinstance(x, str) and eval(x) else '')
    ...
    cols2rearrange = ['fractal_dim', 'center_mass_shift', 'MaxIntensityTumorVolume20%',
                      'MaxIntensityTumorVolume30%', 'MaxIntensityTumorVolume40%', 'MaxIntensityTumorVolume50%',
                      'MaxIntensityTumorVolume60%', 'MaxIntensityTumorVolume70%', 'vmin', 'vmax']
    texture = pd.concat([texture[cols2rearrange], texture.drop(columns=cols2rearrange)], axis=1)

    if if_shape:
        shape = pd.read_csv(path_save + 'shape_' + save_as + '.csv', index_col=0)
        shape['patient'] = shape['patient'].astype(str)
        shape = shape.set_index(['patient', 'organ'])
        # drop not needed columns
        shape = shape.drop(columns=["nonzero_Points", "Clusters"])
        df = shape.merge(texture, left_index=True, right_index=True, how='outer')
    else:
        df = texture

    # put voxels to front
    df.insert(0, 'voxels', df.pop('voxels'))
