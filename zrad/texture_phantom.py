...
class Texture(object):
    def __init__(self, sb, IM,structure, x_ct,y_ct, columns, rows, xCTspace, patientPos, rs, slices, voxel_size, path, ImName, pixNr, binSize, prefix, wv, *cont):#Xc, **Yc, **XcW, **YcW ):
        ...
        self.vmin = []
        self.vmax = []
        rs_type = [1, 0, 0, 0, 0, 0, 0, 0, 0] #structure type, structure resolution, transformed or non-transformed
        ...
        try:
            ...
            if structure != 'none':
                if 'BV' not in names:
                    struct =  Structures(rs, structure, self.slices, self.x_ct,self.y_ct, self.xCTspace, self.position, len(maps[0])) #read the structure points
                    ...
                    self.structure = struct.organs
                    del struct
                    sb.SetStatusText(ImName +' RS done '+ str(datetime.now().strftime('%H:%M:%S')))
                else:
                ...
                self.Xcontour = ''
            if self.Xcontour == 'one slice': #don't calculate
                ...
                for i in arange(0, len(maps)):
                    if wv:
                        ...
                        if 'BV' not in names:
                            ...
                            wave_list = WaveletCTP(maps[i], path, names[i], ImName+'_'+pixNr).Return()
                        sb.SetStatusText(ImName +' wave done ' +str(datetime.now().strftime('%H:%M:%S')))
                        rs_type = [1, 0, 0, 0, 0, 0, 0, 0, 0] #structure tyoe, structure resolution
                    else:
                        rs_type = [1]
                        wave_list = [maps[i]]
                    for w in arange(0, len(wave_list)):
                        try:
                            ...
                            matrix, interval, norm_points, matrix_v = self.Matrix(wave_list[w], rs_type[w], structure)
                            try:
                                sb.SetStatusText(ImName +' matrix done '+ str(datetime.now().strftime('%H:%M:%S')))
                            except AttributeError:
                                pass
                            if rs_type[w]: #save only for the original image
                                self.saveImage(path, names[i], matrix, ImName, pixNr)
                            more_than_one_pix = True
                            try:
                            #histogram
                                histogram = self.fun_histogram(matrix_v, names[i], ImName, pixNr, path, w)
                            except IndexError:
                            ...
                            if more_than_one_pix:
                                ...
                                try:
                                    ...
                                    pass
                                self.cms.append(self.centerMassShift(matrix_v, voxel_size))
                                del matrix_v
                                #coocurence matrix
                                energy_t=[]
                                entropy_t=[]
                                ...
                                list_GLRLM_merged = []
                                for c in lista_t:
                                    GLRLM, GLRLM_norm = self.M4L(matrix, c, lista_t)
                                ...
                                self.strength.append(round(strength,3))
                                try:
    ...
    def ret(self):
        '''return function'''                             
        return (self.vmin, self.vmax, self.structure, self.mean, self.std, self.cov, self.skewness, self.kurtosis, self.var, self.median, self.percentile10, self.percentile90, self.iqr, self.range, self.mad, self.rmad, self.H_energy, self.H_entropy, self.rms, self.H_uniformity,
                self.energy, self.entropy, self.contrast, self.correlation, self.homogenity, self.homogenity_n, self.idiff, self.idiff_n, self.variance, self.sum_average, self.sum_entropy, self.sum_variance, self.diff_entropy, self.diff_variance, self.IMC1, self.IMC2, self.MCC,self.joint_max, self.joint_average,self.diff_ave,self.dissim,self.inverse_var,self.autocorr,self.clust_t,self.clust_s, self.clust_p,
                self.COM_energy, self.COM_entropy, self.COM_contrast, self.COM_correlation, self.COM_homogenity, self.COM_homogenity_n, self.COM_idiff, self.COM_idiff_n, self.COM_variance, self.COM_sum_average, self.COM_sum_entropy, self.COM_sum_variance, self.COM_diff_entropy, self.COM_diff_variance, self.COM_IMC1, self.COM_IMC2, self.COM_MCC,self.COM_joint_max, self.COM_joint_average,self.COM_diff_ave,self.COM_dissim,self.COM_inverse_var,self.COM_autocorr,self.COM_clust_t,self.COM_clust_s,self.COM_clust_p,
                self.coarseness, self.neighContrast, self.busyness, self.complexity, self.strength,
                self.len_intensity,self.len_intensity_n, self.len_size, self.len_size_n, self.len_sse, self.len_lse, self.len_lgse, self.len_hgse, self.len_sslge, self.len_sshge, self.len_lslge, self.len_lshge, self.len_rpc, self.len_glv, self.len_lsv, self.len_size_entropy,
                self.M_len_intensity ,self.M_len_intensity_n, self.M_len_size, self.M_len_size_n, self.M_len_sse, self.M_len_lse, self.M_len_lgse, self.M_len_hgse, self.M_len_sslge, self.M_len_sshge, self.M_len_lslge, self.M_len_lshge, self.M_len_rpc, self.M_len_glv, self.M_len_lsv, self.M_len_size_entropy,
                self.intensity, self.intensity_n, self.size, self.size_n, self.sse , self.lse, self.lgse, self.hgse, self.sslge, self.sshge, self.lslge, self.lshge, self.rpc, self.glv, self.lsv, self.size_entropy,
                self.GLDZM_intensity, self.GLDZM_intensity_n, self.GLDZM_size, self.GLDZM_size_n, self.GLDZM_sse , self.GLDZM_lse, self.GLDZM_lgse, self.GLDZM_hgse, self.GLDZM_sslge, self.GLDZM_sshge, self.GLDZM_lslge, self.GLDZM_lshge, self.GLDZM_rpc, self.GLDZM_glv, self.GLDZM_lsv, self.GLDZM_size_entropy,
                self.NGLDM_intensity, self.NGLDM_intensity_n, self.NGLDM_size, self.NGLDM_size_n, self.NGLDM_sse , self.NGLDM_lse, self.NGLDM_lgse, self.NGLDM_hgse, self.NGLDM_sslge, self.NGLDM_sshge, self.NGLDM_lslge, self.NGLDM_lshge, self.NGLDM_glv, self.NGLDM_lsv, self.NGLDM_size_entropy, self.NGLDM_energy,
                self.frac_dim, self.cms, self.points)
    ...
    def Matrix(self, imap, rs_type, structure):
        ...
        lxmax = []
        v = []
        if structure != 'none': #if the structure if defined
        ...
        if structure != 'none':
            ...
            if self.bits == '':
                ...
                self.n_bits = int((vmax-vmin)/self.binSize)+1
            else:
            ...
            for i in arange(0, len(Xcontour)): #slices
                for j in arange(0, len(Xcontour[i])): #sub-structres in the slice
                    for k in arange(0, len(Xcontour[i][j])):
                        try:
                            matrix[i][Ycontour[i][j][k]-ymin][Xcontour[i][j][k]-xmin] = int((imap[i][Ycontour[i][j][k]][Xcontour[i][j][k]]-vmin)/interval) #first row, second column, changing to self.bits channels as in co-ocurence matrix we have only the channels channels
                            matrix_true[i][Ycontour[i][j][k]-ymin][Xcontour[i][j][k]-xmin] = imap[i][Ycontour[i][j][k]][Xcontour[i][j][k]] #first row, second column, changing to self.bits channels as in co-ocurence matrix we have only the channels channels
                        except ValueError:
                            pass
            ...
            if self.bits == '':
                interval = self.binSize
                self.n_bits = ((vmax-vmin)/self.binSize)+1
            else:
                interval = (vmax-vmin)/(self.bits-1) #matrix with self.bits channels
                self.n_bits = self.bits
            print interval
            ...
            for i in arange(0, len(imap)): #slices
                for j in arange(0, len(imap[i])): #rows
                    for k in arange(0, len(imap[i][j])): #columns
                        try:
                            matrix[i][j-ymin][k-xmin] = int((imap[i][j][k]-vmin)/interval) #first row, second column, changing to self.bits channels as in co-ocurence matrix we have only the channels channels
                            ...
                            pass
                    ind = np.where(np.isnan(imap))[0]
                    norm_points.append(len(imap)*len(imap[0])*len(imap[0][0])-len(ind)) #how many points are used for calculation$
    ...
    def fun_median(self, M1):
        m = np.median(M1)
        return m

    def fun_percentile(self, M1, px):
        p = np.percentile(M1, px)
        return p

    def fun_interqR(self, M1):
        irq = self.fun_percentile(M1, 75) - self.fun_percentile(M1, 25)
        return irq

    def fun_range(self, M1):
    ...
    def fun_H_energy(self, M1):
        e = np.sum(M1**2)
        return e

    def fun_H_entropy(self, M1, interval):
        vmin = np.min(M1)
    ...
    def fun_H_uniformity(self, M1, interval):
        vmin = np.min(M1)
        dM1 = ((M1-vmin)//interval)+1      
    ...
    def coMatrix(self, M,trans):
        ...
        #values at the digonal should be added only once
...
#            coMatrix[i][i] = coMatrix[i][i]/2.
    ...
    def fun_correlation(self, coM): #3.3.19
        ...
        std = 0
        for i in arange(0, len(coM)):
            std += ((i+1 - mean)**2)*np.sum(coM[i])
        std = np.sqrt(std)
            
        ind = np.where(np.array(coM)!=0)
        corr = 0
        for i in arange(0, len(ind[0])):
            corr += (ind[0][i]+1)*(ind[1][i]+1)*coM[ind[0][i]][ind[1][i]]
    ...
    def fun_autocorr(self, coM): #3.3.20
        ...
        ind = np.where(np.array(coM)!=0)
        for i in arange(0, len(ind[0])):
            c += (ind[0][i]+1)*(ind[1][i]+1)*coM[ind[0][i]][ind[1][i]]
        return c
    ...
    def fun_cluster(self, coM): #3.3.21, 3.3.22 and 3.3.23
        ...
        ind = np.where(np.array(coM)!=0)
        for i in arange(0, len(ind[0])):
            clust_t += ((ind[0][i] + ind[1][i] + 2 - 2*mean)**2)*coM[ind[0][i]][ind[1][i]]
    ...
    def M3(self, matrix): #Amadasun et al. Textural Features Corresponding to Textural Properties
        ...
        s = np.zeros(self.n_bits)
        Ni = np.zeros(self.n_bits)
            ...
            for v in arange(0,self.n_bits):
                ...
                for ind in arange(0,len(index[0])):
                    ...
                    for z in [-1,1]:
                        for y in [-1,0,1]:
                            for x in [-1,0,1]:
                                if k+z >= 0 and index[0][ind]+y >= 0 and index[1][ind]+x >= 0 and k+z < len(matrix) and index[0][ind]+y < len(matrix[0]) and index[1][ind]+x < len(matrix[0][0]):
                                    temp.append(matrix[k+z][index[0][ind]+y][index[1][ind]+x])
                                else:
                                    numerator+=1
                    for y in [-1,1]:
                        for x in [-1,0,1]:
                            if index[0][ind]+y >= 0 and index[1][ind]+x >= 0 and index[0][ind]+y < len(matrix[0]) and index[1][ind]+x < len(matrix[0][0]):
    ...
    def fun_strength(self, s, Ni, matrix):
        ind = np.where(np.array(Ni)!=0)[0]
        s1 = 0
        for i in ind:
            for j in ind:
    ...
    def M4L(self, matrix, di, direction): 
        ...
        vector_len = max([len(matrix), len(matrix[0]), len(matrix[0][0]),
            int(np.sqrt(len(matrix)**2+len(matrix[0])**2))+1, int(np.sqrt(len(matrix)**2+len(matrix[0][0])**2))+1,int(np.sqrt(len(matrix[0][0])**2+len(matrix[0])**2))+1])
        ...
        if np.sum(di)!=1 or di[0]*di[1]*di[2] != 0:
            ...
            if di[2] == -1:
                vm = np.concatenate((vm, self.M4L_ray(matrix, seeds, di, v, shiftX = 1, shiftY = 0, shiftZ = 0)))
            else:
                vm = np.concatenate((vm, self.M4L_ray(matrix, seeds, di, v, shiftX = -1, shiftY = 0, shiftZ = 0)))
            if di in direction[5:]:
                vm = np.concatenate((vm, self.M4L_ray(matrix, seeds, di, v, shiftX = 0, shiftY = 0, shiftZ = -1)))
            if di in direction[9:]:
                if di[2] == -1:
                    vm = np.concatenate((vm, self.M4L_ray(matrix, seeds, di, v, shiftX = 1, shiftY = 0, shiftZ = -1)))
                else:
                    vm = np.concatenate((vm, self.M4L_ray(matrix, seeds, di, v, shiftX = -1, shiftY = 0, shiftZ = -1)))
        ...
        #test
##        print di
##        for i in arange(0, len(GLRLM)):
##            suma = 0
##            for j in arange(0, len(GLRLM[0])):
##                suma += (j+1)*GLRLM[i][j]
##            print i+1, suma
##        print '\n'
            ...
            if np.sum(d) == 1:
                for z in arange(0, len(matrix)+1):
                    for cor in arange(-max_dim, max_dim):
                        seeds.append([z+corner[0],(cor+corner[1])*abs(d[1]-1),(cor+corner[2])*abs(d[2]-1)])
            else:
                for z in arange(0, len(matrix)+1):
                    for cor in arange(-max_dim, max_dim):
...
##                            p = []
##                            for n in arange(0, len(points)):
##                                m[points[n][0]][points[n][1]][points[n][2]] = np.nan
##                                poin = self.neighbor(points[n][0],points[n][1],points[n][2], m,v)
##                                for ni in poin:
##                                    if ni not in zone:
##                                        zone.append(ni)
...
##                            points = p
        ...
        GLSZM = np.array(GLSZM)#no normalization/float(np.sum(GLSZM))
        norm_GLSZM = np.sum(GLSZM)
        GLSZM.astype(np.float)

        GLDZM = np.array(GLDZM)#no normalization/float(np.sum(GLSZM))
    ...
    def fun_NGLDM_energy(self, NGLDM, norm):
