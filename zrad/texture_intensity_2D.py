...
class Intensity(object):
    ...
    def feature_calculation(self, interval):
        ...
        for i in np.arange(1, len(ind1) + 1):
            temp.pop(ind1[-i])
        ind2 = np.where(np.array(temp) > p90)[0]
        for i in np.arange(1, len(ind2) + 1):
            temp.pop(ind2[-i])
...
class GLCM(object):
    ...
    def matrix_calculation(self):
        ...
        for i in np.arange(0, len(matrix)):  # for every slice
            for y in np.arange(0, len(matrix[i])):  # for every row in slice i
                for x in np.arange(0, len(matrix[i][y])):  # for every column in row y of slice i
                    for d in np.arange(0, len(directions)):  # 4 directions (0-3) in 2D
    ...
    def norm_marginal_calculation(self, glcm_matrix):
        ...
        """
        # normalize matrix
        for dd in np.arange(0, self.nrdirections):
            ...
            for i in np.arange(0, len(matrix_t)):
            ...
            for ax0 in np.arange(0, len(norm_matrix)):  # for each slice
                for ax1 in np.arange(0, len(norm_matrix[ax0])):
                    for ax2 in np.arange(0, len(norm_matrix[ax0][ax1])):
    ...
    def glcm_feature_calculation(self):
        ...
        for dd in np.arange(0, self.nrdirections):
            ...
            for x in np.arange(0, nrslices):  # matrix[x] is 2dimensional
                # 3.6.1 Joint Maximum
                self.joint_max.append(np.max(matrix[x]))

                # 3.6.2 Joint Average
                s = 0
                for i in np.arange(0, len(matrix[x])):
                ...
                for j in np.arange(0, len(ind[0])):
                ...
                a = 0
                v = 0
                for k in np.arange(0, len(self.p_minus[dd][x])):
                ...
                e = 0
                for i in np.arange(0, len(self.p_minus[dd][x])):
                ...
                a = 0
                v = 0
                for k in np.arange(2, len(self.p_plus[dd][x])):
                ...
                # 3.6.10 sum entropy
                e = 0
                for i in np.arange(2, len(self.p_plus[dd][x])):
                ...
                f = 0
                for i in np.arange(0, len(matrix[x])):
                    for j in np.arange(i + 1, len(matrix[x][0])):
                ...
                hxy2 = 0
                for i in np.arange(0, len(matrix[x])):
                    for j in np.arange(0, len(matrix[x][i])):
                        if X[i] * X[j] != 0:
...
class GLRLM_GLSZM_GLDZM_NGLDM(object):
    ...
    def matrix_calculation(self):
        if self.matrix_type == "GLRLM":
            ...
            for d in np.arange(0, len(self.directionvector)):
                m = np.copy(self.matrix)
                for z in np.arange(0, nr_slices):
                    for y in np.arange(0, len(m[z])):  # for every row
                        for x in np.arange(0, len(m[z][y])):  # for every column
                            grey_value1 = m[z][y][x]
        ...
        if self.matrix_type == "GLSZM" or self.matrix_type == "GLDZM":
            ...
            for i in np.arange(0, self.n_bits):
                GLSZM.append([0])
                GLDZM.append([0])
            GLSZM_perslice = np.zeros((len(m), self.n_bits, Smax))
            GLDZM_perslice = np.zeros((len(m), self.n_bits, Dmax))
            for k in np.arange(0, len(self.matrix)):
                for i in np.arange(0, len(self.matrix[k])):
                    for j in np.arange(0, len(self.matrix[k][i])):
                        if np.isnan(m[k][i][j]):  # skip nan values
                            ...
                            while len(points) != 0:  # LOOK FOR NEIGHBORS OF NEIGHBORS
                                p = []
                                for n in np.arange(0, len(points)):
                            ...
                            if size > Smax:  # check if matrix (max zone size) needs to be enlarged
                                for s in np.arange(0, len(GLSZM)):
                                    for si in np.arange(0, size - Smax):
                                        GLSZM[s].append(0)
                            ...
                            if min_distance > Dmax:
                                for s in np.arange(0, len(GLDZM)):
                                    for si in np.arange(0, min_distance - Dmax):
                                        GLDZM[s].append(0)
            ...
            for i in np.arange(0, len(GLSZM)):
        ...
        if self.matrix_type == "NGLDM":
            ...
            for k in np.arange(0, len(matrix)):
                for v in np.arange(0, self.n_bits):
                    index = np.where(matrix[k] == v)  # search for a value level
                    for ind in np.arange(0, len(index[0])):
                        temp = []
                        numerator = 0
                        # enable for 3D
                        # for z in [-1, 1]:
                        ...
                        for n in np.arange(1, len(ind_nan) + 1):  # remove nan neighbours
                            temp.pop(ind_nan[-n])
                        ...
                        if numerator != 8:  # 8 would mean no neighbours in 2D
                            ...
                            if size > maxSize:
                                for gray in np.arange(0, len(s)):
                                    for app in np.arange(maxSize, size):
                                        s[gray].append(0)
    ...
    def neighbor(self, z, y, x, matrix, v):
        ...
        points = []
        for k in np.arange(0, 1):  # don't look for neighbors in z direction for 2D
    ...
    def feature_calculation(self, matrix_dimension_type, matrix_method):
        ...
        for dd in np.arange(0, nrdirection):  # for all directions
            matrix = matrix_t[dd]

            for x in np.arange(0, nrslices):
                ...
                lgse = 0
                for i in np.arange(0, len(matrix[x])):  # grey value
                    for j in np.arange(0, len(matrix[x][i])):  # run length
                ...
                for m in np.arange(0, len(matrix[x])):
                    s = 0
                    for n in np.arange(0, len(matrix[x][m])):
                        s += matrix_tt[m][n]
                ...
                for n in np.arange(0, len(matrix_temp[0])):
                    s = 0
                    for m in np.arange(0, len(matrix_temp)):
                        s += matrix_temp[m][n]
                ...
                # nv = 0
                # for j in np.arange(0, len(matrix[x][0])):  # loop over column
...
class NGTDM(object):
    ...
    def ngtdm_matrix_calculation(self):
        ...
        if self.dim == "2D" or self.dim == "2D_singleSlice":
            ...
            max_neighbours = 26
        for k in np.arange(0, nr_slices):
            for v in np.arange(0, self.n_bits):  # for each grey level
                index = np.where(matrix[k] == v)  # search for a value level
                for ind in np.arange(0, len(index[0])):    # for each element of grey value v look at neighbors
                    temp = []
                    numerator = 0
                    ...
                    for n in np.arange(1, len(ind_nan) + 1):
                        temp.pop(ind_nan[-n])
                    numerator += len(ind_nan)
    ...
    def feature_calculation(self, method):
        ...
        for x in np.arange(0, len(s)):  # len s = nr slices or =1 for 3d
            # 3.10.1 coarseness
            f = 0
...
class CMS_MTV(object):
    ...
    def center_mass_shift(self):  # shift it into shape features!!!!!!!!!!!!!!!! ?????????????????????
        ...
        if self.dim == "2D" or self.dim == "2D_singleSlice":
            ...
            cms_2d = []
            for s in np.arange(0, len(self.matrix)):
    ...
    def fractal(self):  # check if this is corrclty implemented for 2 D cases!!!!!!!!
        ...
        try:
            ...
            frac = []
            for r in np.arange(2, maxR + 1):  # because log(1) = 0
                N = 0
                if self.dim == "2D":  # in 2D several slices case, we want to iterate through each slice.
                    ...
                    rz = r
                    rzz = r
                for z in np.arange(0, len(m), rz):  #
                    for y in np.arange(0, len(m[0]), r):
                        for x in np.arange(0, len(m[0][0]), r):
                            m = np.array(m)
