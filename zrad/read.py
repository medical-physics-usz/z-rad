...
class ReadImageStructure(object):
    ...
    *modality - optional argument, list of prefixes in the fucntional maps for example for CTP modality = ['BV', 'MTT', 'BF']
    '''   
    ...
    def ReadImages(self, UID, mypath_image):
        ...
        if 'CTP' in UID or 'IVIM' in UID: # CTP and IVIM
            onlyfiles = []
            for u in UID: #interate through different maps types in te functional imaging
                of = []
    ...
    def ReadStucture(self, mypath_image, structure, wv, local):
        ...
        struct = Structures(self.rs, structure, self.slices, self.x_ct, self.y_ct, self.xCTspace, len(self.slices),wv, local)
