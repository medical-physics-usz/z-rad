...
class LymphNodes(object):
    ...
    def __init__(self, name_ln, name_sh, path_files_shape, path_save, path_save_dir, start, stop):
        ...
        for ImName in range(start, stop+1):
            ...
            if tumorMass == "": #no such a file or directory
                final.append((ImName, "", "", "", "", "", "", "", "", ""))
                continue  
            listLnMass, listLnVol, listLnPoints = self.FindLN(name_ln, path_files_shape, ImName) #returs list of lm center of the mass, volumes and all LN points
            ...
            volLargestWeightedDist, volMeanWeightedDist, normVolLargestWeightedDist, normVolMeanWeightedDist = self.VolWeightedDist(distances, listLnVol, ptVol)
            ...
            final.append((ImName, largestDist, meanDist, sumDist, weightedDist, volLargestWeightedDist, volMeanWeightedDist, normVolLargestWeightedDist, normVolMeanWeightedDist, massPtLn, minDistMean, minDistVar, nrClusters, elong, flat))
        #save final file
    ...
    def FindMassCenter(self, name_sh, path_files_shape, ImName): 
        ...
        try:
            ...
            for z in range(min(l_slices),max(l_slices)+1):
                try:
                    data = np.loadtxt(path_files_shape+name_sh+'\\'+ImName+'\\slice_'+str(z)) #if the file exists
        ...
        except WindowsError: #no such a directory (in case this patient has less LN than the others)
            MassCenter = ''
            volume = ''
            ind = ''
    ...
    def FindLN(self, name_ln, path_files_shape, ImName): #returs list of lm center of the mass
        ...
        onlydirs =[f for f in listdir(path_files_shape) if f.startswith(name_ln) and isdir(join(path_files_shape,f))] #find only folder correpsonding to lymph nodes
        listLnMass = []
        listLnPoints = []
        listLnVol = []
        ...
        if len(listLnMass)>1: #if more than one LN
            for mainLN in listLnMass: #distances between LNs
                ...
                for ln in listLnMass:
                    ...
                    distance.append(np.sqrt(float(dist)))
                listSumDist.append(np.sum(distance)/(len(listLnMass)-1)) #divide by the number of LN -1 to noramlize the sum of distances
    ...
    def SmallestDist(self, tumorMass, listLnMass, path, nr):
        ...
        if len(points) > 4: #convex hull needs at least 5 points
            ...
            Tcsr = minimum_spanning_tree(distMatrix) #https://docs.scipy.org/doc/scipy-0.15.1/reference/generated/scipy.sparse.csgraph.minimum_spanning_tree.html
        ...
        try:
            ...
            py.title(nr)
            try:
                makedirs(path+'\\LN_PT_min_distance_distribution\\')
            except OSError:
                if not isdir(path+'\\LN_PT_min_distance_distribution\\'):
                    raise
            fig.savefig(path+'\\LN_PT_min_distance_distribution\\'+nr+'.png')
            py.close()
...
##        return area 
    ...
    def Clusters(self, points, path, nr):
        ...
        if len(points) == 1: #only primary
            ...
            py.title(nr)
            try:
                makedirs(path+'\\LN_PT_clustering\\')
            except OSError:
                if not isdir(path+'\\LN_PT_clustering\\'):
                    raise
            fig.savefig(path+'\\LN_PT_clustering\\'+nr+'.png')
            py.close()
...
#        #remove possible overlapping points
    ...
    def Save(self, final, path_save):
        '''export results
        final - list with results'''
        names = ['patient', 'largestDist', 'meanDist', 'sumDist', 'weightedDist', 'volLargestWeightedDist', 'volMeanWeightedDist', 'normVolLargestWeightedDist', 'normVolMeanWeightedDist', 'massPtLn', 'minDistMean', 'minDistVar', 'optClust', 'elongation', 'flateness']
