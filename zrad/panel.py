...
class MyPanelResize(scrolled.ScrolledPanel):
    ...
    def InitUI(self):   
        ...
        h = self.parent.panelHeight #hight of a text box, 20 for PC, 40 for lenovo laptop
        self.SetBackgroundColour('#8AB9F1') #background color
        
        #creatign boxes conataig elements of the panel, vbox - veritcal box, hbox - horizontal box
        ...
        st_org = wx.StaticText(self, label = u'Original data') #statitc text        
        tc_org = wx.TextCtrl(self, id = 1001,  size = (600, h),value = "", style = wx.TE_PROCESS_ENTER) #text box, id is important as I use i later for reading elements from the boxes
        #tc_org- directory with original images
        btn_load_org = wx.Button(self, -1, label = 'Search') #button to search
        st_save_resized = wx.StaticText(self, label = u'Save resized files') 
        tc_save_resized = wx.TextCtrl(self, id = 1002,  size = (600, h),value = "", style = wx.TE_PROCESS_ENTER) #directory to ave resized images
        btn_load_resized = wx.Button(self, -1, label = 'Search') 
        st_name = wx.StaticText(self, label = u'Structure name') 
        tc_name = wx.TextCtrl(self, id = 1003,  size = (600, h),value = "", style = wx.TE_PROCESS_ENTER) #structures to be resized eparated by coma ','
        st_reso = wx.StaticText(self, label = u'Resolution for texture calculation [mm]') 
        tc_reso = wx.TextCtrl(self, id = 1004,  size = (100, h),value = "", style = wx.TE_PROCESS_ENTER) #resoltuion for texture calculation
        st_type = wx.StaticText(self, label = u'Image type') 
        tc_type = wx.ComboBox(self, id = 1005,  size = (100, h), value = "",choices = ['CT', 'PET', 'MR', 'IVIM'], style = wx.TE_PROCESS_ENTER) #modality type
        st_start = wx.StaticText(self, label = u'Start')
        tc_start = wx.TextCtrl(self, id = 1006,  size = (100, h),value = "", style = wx.TE_PROCESS_ENTER) #patient number to start
        st_stop = wx.StaticText(self, label = u'Stop') 
        tc_stop = wx.TextCtrl(self, id = 1007,  size = (100, h),value = "", style = wx.TE_PROCESS_ENTER) #patient number to stop
        
        cb_cropStructure = wx.CheckBox(self, id = 1012, label = u'Use CT Structure') 
        st_cropStructure = wx.StaticText(self, label = u'     CT Path') #statitc text        
        tc_cropStructure = wx.TextCtrl(self, id = 1013,  size = (600, h),value = "", style = wx.TE_PROCESS_ENTER) #text box, id is important as I use i later for reading elements from the boxes
        #tc_org- directory with original images
        btn_cropStructure = wx.Button(self, -1, label = 'Search') #button to search
        
        cb_texture = wx.CheckBox(self, id = 1008, label = u'Resize texture')
        cb_shape = wx.CheckBox(self, id = 1009, label = u'Resize shape')
        ...
        self.gs_01.AddMany([st_org, tc_org, btn_load_org,wx.StaticText(self,  label = ''),
                           st_save_resized, tc_save_resized, btn_load_resized,wx.StaticText(self,  label = ''),
                            ...
                            cb_texture, wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''),
                            cb_shape, wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''), wx.StaticText(self,  label = '') ,                           
                            cb_cropStructure, wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''),wx.StaticText(self,  label = ''),
                            st_cropStructure, tc_cropStructure, btn_cropStructure,wx.StaticText(self,  label = ''),
                            wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''),wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''),
                            wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''), btn_check, wx.StaticText(self,  label = ''),
                            wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''), btn_resize, wx.StaticText(self,  label = '')])
    ...
    def OnOpenOrg(self, evt): #need and event as an argument
    ...
    def OnOpenR(self, evt): #need and event as an argument
    ...
    def resize(self, evt): #need and event as an argument
        ...
        if self.FindWindowById(1008).GetValue(): #if resizing to texture reusltion selected
            ResizeTexture(inp_resolution,list_structure, inp_mypath_load, inp_mypath_save, image_type, begin, stop, cropInput) #resize images and structure to the resultion of texture
        
        if self.FindWindowById(1009).GetValue(): #if resizing to shape reusltion selected
            inp_mypath_save_shape = inp_mypath_save +'\\resized_1mm\\'
               
            for shape_struct in list_structure:
                ResizeShape(shape_struct, inp_mypath_load, inp_mypath_save_shape, image_type, begin, stop, inp_resolution, cropInput) # resize the structure to the resoltuion of shape, default 1mm unless resolution of texture smaller tham 1mm then 0.1 mm
    ...
    def fill(self, l):
        ...
        ids = [1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008, 1009, 1012, 1013] #ids of field to fill # if adjust number of ids then also adjust in main_texture in self.panelResize.fill(l[:11])!!!!
    ...
    def save(self):
        ...
        l = []
        ids = [1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008, 1009, 1012, 1013]
        for i in ids:
...
class MyPanelRadiomics(scrolled.ScrolledPanel):
    ...
    def InitUI(self):
        ...
        st_path = wx.StaticText(self, label = u'Load') 
        tc_path = wx.TextCtrl(self, id = 107,  size = (900, h),value = "", style = wx.TE_PROCESS_ENTER) #path to resized images
        btn_load_path = wx.Button(self, -1, label = 'Search') #button to open dialog box
        st_pref = wx.StaticText(self, label = u'Prefix') 
        tc_pref= wx.TextCtrl(self, id = 108,  size = (400, h),value = "", style = wx.TE_PROCESS_ENTER) #prefix in the name of patients folders for example HN, goes to HN_X
        st_range_l = wx.StaticText(self, label = u'Start') 
        tc_range_l= wx.TextCtrl(self, id = 109,  size = (400, h),value = "", style = wx.TE_PROCESS_ENTER) #patient number start
        st_range_u = wx.StaticText(self, label = u'Stop') 
        ...
        st_saver = wx.StaticText(self, label = u'Save results in') 
        ...
        st_saveas = wx.StaticText(self, label = u'Save as') 
        tc_saveas = wx.TextCtrl(self, id = 103,  size = (400, h),value = "", style = wx.TE_PROCESS_ENTER) #name of the text file to save texture results
        ...
        st_name = wx.StaticText(self, label = u'Structure name ')
        tc_name = wx.TextCtrl(self, id = 104,  size = (200, h),value = "", style = wx.TE_PROCESS_ENTER) #name of ROI to be analysed, spearate by ',';  take the first structure which can be found in the RS
        st_disc = wx.StaticText(self, label = u'Discretization')
        st_bin = wx.StaticText(self, label = u'Bins No')
        st_size = wx.StaticText(self, label = u'Bin size')
        tc_bin = wx.TextCtrl(self, id =1051,  size = (200, h),value = "", style = wx.TE_PROCESS_ENTER) #size of the bin
        tc_size = wx.TextCtrl(self, id =1052,  size = (200, h),value = "", style = wx.TE_PROCESS_ENTER) #number of bins for discetization
        st_dim = wx.StaticText(self, label = u'Dimension')
        rb_dim2 = wx.RadioButton(self, id = 1061, label = u'2D', style = wx.RB_GROUP) #2D analysis, radiobutton - only one option from the group can be selected
        rb_dim3 = wx.RadioButton(self, id =1062,  label = u"3D") #3D analysis
        st_wv = wx.StaticText(self, label = u'Wavelet transform ')
        rb_wvT = wx.RadioButton(self, id = 1071, label = u'On', style = wx.RB_GROUP) #wavelet on, radiobutton - only one option from the group can be selected
        rb_wvF = wx.RadioButton(self, id = 1072,  label = u"Off") #wavelet off
        st_s = wx.StaticText(self, label = u'Shape ')
        rb_sT = wx.RadioButton(self, id = 1081, label = u'On', style = wx.RB_GROUP) #shape on, radiobutton - only one option from the group can be selected
        rb_sF = wx.RadioButton(self, id =1082,  label = u"Off") #shape off 
        st_name_shape = wx.StaticText(self, label = u'Structure name shape ')
        tc_name_shape = wx.TextCtrl(self, id =1083,  size = (200, h),value = "", style = wx.TE_PROCESS_ENTER)
        st_LN = wx.StaticText(self, label = u'Lymph nodes ')
        rb_LNT = wx.RadioButton(self, id = 1091, label = u'On', style = wx.RB_GROUP) #analysis of distribution of LN around PT on, radiobutton - only one option from the group can be selected
        rb_LNF = wx.RadioButton(self, id =1092,  label = u"Off") #analysis of distribution of LN around PT off
        st_name_LN = wx.StaticText(self, label = u'Name lymph node structure ')
        tc_name_LN = wx.TextCtrl(self, id =1093,  size = (200, h),value = "", style = wx.TE_PROCESS_ENTER) #name of LN structure to be analysed, for example g_LN will search for g_LN_X
        ...
        st_mod = wx.StaticText(self, label = u'Modality')
        ...
        st_suv = wx.StaticText(self, label = u'SUV correction')
        rb_yes = wx.CheckBox(self, id = 131, label = u'yes') #SUV correciton on
        rb_no = wx.CheckBox(self, id = 132, label = u'no') #SUV correciton off
        
        crop_text = wx.StaticText(self, label = u'Use CT Structure') #crop PET structure to CT structure set HU range
        crop_yes = wx.CheckBox(self, id = 133, label = u'yes') #SUV correciton on
        crop_no = wx.CheckBox(self, id = 134, label = u'no') #SUV correciton on
        
        crop_st_humin = wx.StaticText(self, label = u'HU min')
        crop_tc_humin = wx.TextCtrl(self, id = 135,  size = (200, h),value ='', style = wx.TE_PROCESS_ENTER) #HU range min
        crop_st_humax = wx.StaticText(self, label = u'HU max')
        crop_tc_humax = wx.TextCtrl(self, id = 136,  size = (200, h),value ='', style = wx.TE_PROCESS_ENTER) #HU range max
        crop_ctpathlabel = wx.StaticText(self, label = u'CT Path')
        crop_ctpath = wx.TextCtrl(self, id = 137,  size = (400, h),value = "", style = wx.TE_PROCESS_ENTER) #path to resized images
        crop_btn_load_path = wx.Button(self, -1, label = 'Search') #button to open dialog box
        self.gs_6.AddMany([rb_pet, wx.StaticText(self,  label = ''),wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''),
            ...
            wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''), crop_st_humin, crop_tc_humin, wx.StaticText(self,  label = ''), 
            wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''), crop_st_humax, crop_tc_humax, wx.StaticText(self,  label = ''),
            wx.StaticText(self,  label = ''), wx.StaticText(self,  label = ''), crop_ctpathlabel, crop_ctpath, crop_btn_load_path])
        ...
        st_suv = wx.StaticText(self, label = u'outliers correction')
        rb_yes = wx.CheckBox(self, id = 141, label = u'yes') #outlier correction correciton on
        rb_no = wx.CheckBox(self, id = 142, label = u'no')

        self.gs_7.AddMany([rb_ctp, wx.StaticText(self,  label = ''),wx.StaticText(self,  label = ''),wx.StaticText(self,  label = ''),
        ...
        st_stan = wx.StaticText(self, label = u'ROI for standardization') 
        tc_struct1 = wx.TextCtrl(self, id = 151,  size = (200, h),value ='', style = wx.TE_PROCESS_ENTER) #names of sturctures for linaer function fitting to normalize MR
        tc_struct2 = wx.TextCtrl(self, id = 152,  size = (200, h),value ='', style = wx.TE_PROCESS_ENTER)

        self.gs_8.AddMany([rb_mr, wx.StaticText(self,  label = ''),wx.StaticText(self,  label = ''),wx.StaticText(self,  label = ''),
        ...
        self.gs_9.AddMany([rb_ivim, wx.StaticText(self,  label = ''),wx.StaticText(self,  label = ''),wx.StaticText(self,  label = '')])
    ...
    def fill(self, l):
        ...
        ids = [107, 108, 109, 110, 102, 103, 104, 1051, 1052, 1061, 1062, 1071, 1072,1081, 1082, 1083,1091, 1092, 1093, 120, 125, 126, 127, 130, 131, 132, 133, 135, 136, 137,  140, 141, 142,150,151,152,160]
    ...
    def save(self):
        ...
        for i in ids:
#            try:
#                if self.FindWindowById(i).GetValue()[-1] == '\n':
#                    l.append(self.FindWindowById(i).GetValue()[:-1])
#                else:
#                    l.append(self.FindWindowById(i).GetValue())
#            except: 
#                l.append(self.FindWindowById(i).GetValue())
